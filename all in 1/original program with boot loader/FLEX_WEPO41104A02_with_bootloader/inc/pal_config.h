 ...
 *
 * $Id: pal_config.h 17912 2009-09-14 07:49:34Z sschneid $
 *
 ...
 * TRX_TSTAMP_IRQ_HDLR_IDX       Time stamping interrupt
 */
...
#define TRX_PORT2_DDR                   (DDRC)

/*
 * PORTB is transceiver port
 */
#define TRX_PORT1                       (PORTB)
#define TRX_PORT2                       (PORTC)
...
/*
 * SPI Bus Master Output/Slave Input pin is is PORTB 2
 */
#define MOSI                            (PORTB5)
...
/* Wait for 500 ns. */
#define PAL_WAIT_500_NS()               asm volatile("nop\n\t" "nop\n\t" "nop\n\t" "nop\n\t" ::);
 ...
 */
#define SS_LOW()                        ((TRX_PORT2) &= (~SS_BIT_MASK))

/*
 * Slave select made high
 */
#define SS_HIGH()                       ((TRX_PORT2) |= (SS_BIT_MASK))

/*
 * Mask for SPIF bit in SPSR register
 */
#define SPIF_MASK                       (1 << SPIF0)

/*
 * Wait for SPI interrupt flag
 */
#define SPI_WAIT()                      while (!(SPSR0 & SPIF_MASK))

/*
 * Dummy value written in SPDR0 to retrieve data form it
 */
...
#define TRX_INIT()                      do {                        \
    ...
    SPSR0 = _BV(SPI2X0); /* Double the SPI clock frequency */         \
     ...
     */                                                             \
    SPCR0 &= ~(_BV(SPR00) | _BV(SPR10));                               \
                                                                    \
    /* Set SEL pin which is on Porty to high */                     \
    TRX_PORT2 |= _BV(SEL);                                          \
} while (0)
 ...
 * Alert indication
 */
#define ALERT_INDICATE()                (LED_PORT = ~(LED_PORT))
